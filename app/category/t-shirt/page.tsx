...
export default function TShirtPage() {
  const products = Array.from({ length: 20 }, (_, i) => ({
    ...
    name: 'Zone T-Shirt',
    price: 19.95 + i * 2,
    oldPrice: i % 3 === 0 ? 69.95 : null,
    badge: i % 5 === 0 ? 'Hot' : i % 6 === 0 ? '-30%' : null,
    image: `https://via.placeholder.com/400x500/f0fdf4/22c55e?text=T-Shirt+${i + 1}`,
  }));
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-5xl mx-auto">
            <div className="bg-gray-100 rounded-3xl overflow-hidden shadow-2xl">
              <img src="https://via.placeholder.com/600x800/000000/ffffff?text=Black+T-Shirt" alt="Black T-Shirt" className="w-full object-cover" />
            </div>
            <div className="bg-gray-200 rounded-3xl overflow-hidden shadow-2xl">
              <img src="https://via.placeholder.com/600x800/ffffff/000000?text=White+T-Shirt" alt="White T-Shirt" className="w-full object-cover" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12 bg-white">
            ...
            <Link href={`/products/${product.id}`} key={product.id} className="group block">
              <div className="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-2xl transition">
                <div className="relative">
                  {product.badge && (
                    <span className={`absolute top-4 right-4 text-white text-xs font-bold px-4 py-2 rounded-full ${product.badge === 'Hot' ? 'bg-orange-500' : 'bg-green-500'}`}>
                      {product.badge}
                    </span>
                  )}
                  <img src={product.image} alt={product.name} className="w-full h-96 object-cover group-hover:scale-105 transition" />
                </div>
        ...
        <div className="text-center mt-16">
          <button className="bg-green-500 text-white px-12 py-4 rounded-full text-lg font-semibold hover:bg-green-600 shadow-lg">
            Load More
          </button>
        </div>
      </div>
